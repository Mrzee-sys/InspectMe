...
const EP_CODES = [
  ...
  { value: 'EP1', label: 'EP1 - Obstructed exit doors' },
  { value: 'EP2', label: 'EP2 - Doors do not open outwards' },
  { value: 'EP3', label: 'EP3 - Missing/faulty panic hardware' },
  { value: 'EP4', label: 'EP4 - Combustible materials in escape routes' },
  { value: 'EP5', label: 'EP5 - Non-compliant signage' },
  { value: 'EP6', label: 'EP6 - Signage not illuminated/visible' },
  { value: 'EP7', label: 'EP7 - Backup lighting failed' },
  { value: 'EP8', label: 'EP8 - Assembly point issues' },
  { value: 'EP9', label: 'EP9 - Other' },
]

const EP_ITEMS = [
  { key: 'ep01', label: 'Are all designated emergency exit doors completely unobstructed on both sides?' },
  { key: 'ep02', label: 'Do all emergency exit doors open outwards in the direction of the escape route?' },
  { key: 'ep03', label: 'Are exit doors fitted with panic hardware that does not require a key to open from the inside?' },
  { key: 'ep04', label: 'Are all escape routes entirely free of combustible materials or temporary storage?' },
  { key: 'ep05', label: 'Are emergency exit signs strictly compliant with SANS 1186 (correct green/white symbolic safety signs)?' },
  { key: 'ep06', label: 'Is the emergency exit signage continuously illuminated and clearly visible from anywhere within the escape path?' },
  { key: 'ep07', label: 'Are backup emergency lighting systems functional and tested to operate during load shedding or power failures?' },
  { key: 'ep08', label: 'Are designated outdoor emergency assembly points clearly marked and safely accessible?' },
]

const ITEM_METADATA = {
  ep01: { label: 'Item 1', svg: '<path stroke-linecap="round" stroke-linejoin="round" d="M3 13h2l2 3h10l2-3h2M3 13v6h18v-6M5 13l2-5h10l2 5" />' },
  ep02: { label: 'Item 2', svg: '<path stroke-linecap="round" stroke-linejoin="round" d="M3 13h2l2 3h10l2-3h2M3 13v6h18v-6M5 13l2-5h10l2 5" />' },
  ep03: { label: 'Item 3', svg: '<path stroke-linecap="round" stroke-linejoin="round" d="M3 13h2l2 3h10l2-3h2M3 13v6h18v-6M5 13l2-5h10l2 5" />' },
  ep04: { label: 'Item 4', svg: '<path stroke-linecap="round" stroke-linejoin="round" d="M3 13h2l2 3h10l2-3h2M3 13v6h18v-6M5 13l2-5h10l2 5" />' },
  ep05: { label: 'Item 5', svg: '<path stroke-linecap="round" stroke-linejoin="round" d="M3 13h2l2 3h10l2-3h2M3 13v6h18v-6M5 13l2-5h10l2 5" />' }
};
...
function ElevatingPlatformsPage() {
  const { user } = useAuth()
  ...
  const [deviations, setDeviations] = useState({
    ep: buildInitialDeviationState(EP_ITEMS),
  })
  ...
  async function handleSubmit(event) {
    ...
    const sectionToLabels = (items, sectionValues, optionsMap) =>
      ...
      const payload = {
        ...
        employee: user.id,
        inspectionType: 'ELEVATING_PLATFORMS', // Matches naming convention
        formPayload: {
          details: {
            area: formData.area,
          ...
          deviations: {
            ep: sectionToLabels(EP_ITEMS, deviations.ep, EP_CODES),
          },
        },
      }
  ...
  return (
    <section className="pb-32 flex flex-col items-center">
      <header className="w-full flex items-center justify-between px-4 pt-3 pb-4 bg-white/40 backdrop-blur-xl border-b border-white/50 shadow-sm rounded-none mb-4">
        <div className="flex items-center gap-3">
          ...
          <div className="flex flex-col">
            ...
            <span className="text-slate-900 font-bold text-base leading-tight">Elevating Platforms</span>
          </div>
        </div>
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-5 px-4 w-full">
        ...
        {renderSection('ep', 'Elevating Platforms', EP_ITEMS, EP_CODES)}
...
export default ElevatingPlatformsPage
