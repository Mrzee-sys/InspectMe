...
const EM_CODES = [
  ...
  { value: 'EM1', label: 'EM1 - Obstructed exit doors' },
  { value: 'EM2', label: 'EM2 - Doors do not open outwards' },
  { value: 'EM3', label: 'EM3 - Missing/faulty panic hardware' },
  { value: 'EM4', label: 'EM4 - Combustible materials in escape routes' },
  { value: 'EM5', label: 'EM5 - Non-compliant signage' },
  { value: 'EM6', label: 'EM6 - Signage not illuminated/visible' },
  { value: 'EM7', label: 'EM7 - Backup lighting failed' },
  { value: 'EM8', label: 'EM8 - Assembly point issues' },
  { value: 'EM9', label: 'EM9 - Other' },
]

const EM_ITEMS = [
  { key: 'em01', label: 'Are all designated emergency exit doors completely unobstructed on both sides?' },
  { key: 'em02', label: 'Do all emergency exit doors open outwards in the direction of the escape route?' },
  { key: 'em03', label: 'Are exit doors fitted with panic hardware that does not require a key to open from the inside?' },
  { key: 'em04', label: 'Are all escape routes entirely free of combustible materials or temporary storage?' },
  { key: 'em05', label: 'Are emergency exit signs strictly compliant with SANS 1186 (correct green/white symbolic safety signs)?' },
  { key: 'em06', label: 'Is the emergency exit signage continuously illuminated and clearly visible from anywhere within the escape path?' },
  { key: 'em07', label: 'Are backup emergency lighting systems functional and tested to operate during load shedding or power failures?' },
  { key: 'em08', label: 'Are designated outdoor emergency assembly points clearly marked and safely accessible?' },
]

const ITEM_METADATA = {
  em01: { label: 'Item 1', svg: '<path stroke-linecap="round" stroke-linejoin="round" d="M3 13h2l2 3h10l2-3h2M3 13v6h18v-6M5 13l2-5h10l2 5" />' },
  em02: { label: 'Item 2', svg: '<path stroke-linecap="round" stroke-linejoin="round" d="M3 13h2l2 3h10l2-3h2M3 13v6h18v-6M5 13l2-5h10l2 5" />' },
  em03: { label: 'Item 3', svg: '<path stroke-linecap="round" stroke-linejoin="round" d="M3 13h2l2 3h10l2-3h2M3 13v6h18v-6M5 13l2-5h10l2 5" />' },
  em04: { label: 'Item 4', svg: '<path stroke-linecap="round" stroke-linejoin="round" d="M3 13h2l2 3h10l2-3h2M3 13v6h18v-6M5 13l2-5h10l2 5" />' },
  em05: { label: 'Item 5', svg: '<path stroke-linecap="round" stroke-linejoin="round" d="M3 13h2l2 3h10l2-3h2M3 13v6h18v-6M5 13l2-5h10l2 5" />' },
  em06: { label: 'Item 6', svg: '<path stroke-linecap="round" stroke-linejoin="round" d="M3 13h2l2 3h10l2-3h2M3 13v6h18v-6M5 13l2-5h10l2 5" />' }
};
...
function EarthmovingPage() {
  ...
  const [deviations, setDeviations] = useState({
    em: buildInitialDeviationState(EM_ITEMS),
  })
  ...
  async function handleSubmit(event) {
    ...
    const sectionToLabels = (items, sectionValues, optionsMap) =>
      ...
      const payload = {
        ...
        employee: user.id,
        inspectionType: 'EARTHMOVING_EQUIPMENT', // Matches naming convention
        formPayload: {
          details: {
            area: formData.area,
          ...
          deviations: {
            em: sectionToLabels(EM_ITEMS, deviations.em, EM_CODES),
          },
        },
      }
  ...
  return (
    <section className="pb-32 flex flex-col items-center">
      <header className="w-full flex items-center justify-between px-4 pt-3 pb-4 bg-white/40 backdrop-blur-xl border-b border-white/50 shadow-sm rounded-none mb-4">
        <div className="flex items-center gap-3">
          ...
          <div className="flex flex-col">
            ...
            <span className="text-slate-900 font-bold text-base leading-tight">Earthmoving (TLBs)</span>
          </div>
        </div>
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-5 px-4 w-full">
        ...
        {renderSection('em', 'Earthmoving (TLBs)', EM_ITEMS, EM_CODES)}
