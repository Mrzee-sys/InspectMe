...
const AQT_CODES = [
  ...
  { value: 'AQT1', label: 'AQT1 - Temperatures outside legally comfortable boundaries' },
  { value: 'AQT2', label: 'AQT2 - LEV systems not clean, certified, or working' },
  { value: 'AQT3', label: 'AQT3 - Ineffective cross-ventilation' },
  { value: 'AQT4', label: 'AQT4 - Dust/mist/fumes exceed Occupational Exposure Limits (OEL)' },
  { value: 'AQT5', label: 'AQT5 - Other' },
]

const AQT_ITEMS = [
  { key: 'aqtTemp', label: 'Are indoor office/warehouse temperatures maintained within legally comfortable boundaries for labour?' },
  { key: 'aqtLev', label: 'Are Local Exhaust Ventilation (LEV) systems clean, certified, and working in dust/chemical zones?' },
  { key: 'aqtVent', label: 'Is cross-ventilation natural or mechanical and moving fresh air effectively across large floor spaces?' },
  { key: 'aqtFumes', label: 'Are dust, mist, and fumes monitored and kept under statutory Occupational Exposure Limits (OEL)?' },
]

const ITEM_METADATA = {
  aqtTemp: { label: 'Comfortable Temperatures', svg: '<path stroke-linecap="round" stroke-linejoin="round" d="M12 3v2m0 14v2m9-9h-2M5 12H3m14.485-7.071l-1.414 1.414M6.93 18.485l-1.414 1.414m12.728 0l-1.414-1.414M6.93 6.929L5.516 5.515M12 15a3 3 0 100-6 3 3 0 000 6z" />' },
  aqtLev: { label: 'LEV Systems Operational', svg: '<path stroke-linecap="round" stroke-linejoin="round" d="M19 11H5m14 0a2 2 0 012 2v6a2 2 0 01-2 2H5a2 2 0 01-2-2v-6a2 2 0 012-2m14 0V9a2 2 0 00-2-2M5 11V9a2 2 0 002-2m0 0V5a2 2 0 012-2h6a2 2 0 012 2v2M7 7h10" />' },
  aqtVent: { label: 'Effective Cross-ventilation', svg: '<path stroke-linecap="round" stroke-linejoin="round" d="M14.828 14.828a4 4 0 01-5.656 0M9 10h.01M15 10h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />' },
  aqtFumes: { label: 'Dust & Fumes Monitored', svg: '<path stroke-linecap="round" stroke-linejoin="round" d="M3 15a4 4 0 004 4h9a5 5 0 10-.1-9.999 5.002 5.002 0 10-9.78 2.096A4.001 4.001 0 003 15z" />' },
};
...
function AqtPage() {
  ...
  const [deviations, setDeviations] = useState({
    aqt: buildInitialDeviationState(AQT_ITEMS),
  })
  ...
  async function handleSubmit(event) {
    ...
    const sectionToLabels = (items, sectionValues, optionsMap) =>
      ...
      const payload = {
        ...
        employee: user.id,
        inspectionType: 'AIR_QUALITY_THERMAL_COMFORT', // Ensure backend supports this type
        formPayload: {
          details: {
            area: formData.area,
          ...
          deviations: {
            aqt: sectionToLabels(AQT_ITEMS, deviations.aqt, AQT_CODES),
          },
        },
      }
  ...
  return (
    <section className="pb-32 flex flex-col items-center">
      <header className="w-full flex items-center justify-between px-4 pt-3 pb-4 bg-white/40 backdrop-blur-xl border-b border-white/50 shadow-sm rounded-none mb-4">
        <div className="flex items-center gap-3">
          ...
          <div className="flex flex-col">
            ...
            <span className="text-slate-900 font-bold text-base leading-tight">Air Quality & Thermal</span>
          </div>
        </div>
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-5 px-4 w-full">
        ...
        {renderSection('aqt', 'Thermal Comfort, Ventilation & Indoor Air Quality', AQT_ITEMS, AQT_CODES)}
