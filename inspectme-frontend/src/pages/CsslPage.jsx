...
const CSSL_CODES = [
  ...
  { value: 'CR1', label: 'CR1 - No distinct gender rooms / privacy issues' },
  { value: 'CR2', label: 'CR2 - Overcrowded / poorly ventilated' },
  { value: 'CR3', label: 'CR3 - Lockers dirty / insecure / have damp clothes or food' },
  { value: 'CR4', label: 'CR4 - Benches missing / unstable' },
  { value: 'CR5', label: 'CR5 - Other' },
]

const CSSL_ITEMS = [
  { key: 'CSSLGender', label: 'Are distinct change rooms provided separated by gender where privacy is required for clothing changes?' },
  { key: 'CSSLSpace', label: 'Do change rooms have sufficient, well-ventilated space for workers to change without overcrowding?' },
  { key: 'CSSLLockers', label: 'Is personal storage (lockers) clean, secure, and clear of damp clothes, tools, or food wastes?' },
  { key: 'CSSLBenches', label: 'Are stable benches or seating provided to ensure safe changing into heavy protective safety footwear?' },
]

const ITEM_METADATA = {
  CSSLGender: { label: 'Gender Separation', svg: '<path stroke-linecap="round" stroke-linejoin="round" d="M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197M13 7a4 4 0 11-8 0 4 4 0 018 0z" />' },
  CSSLSpace: { label: 'Space & Ventilation', svg: '<path stroke-linecap="round" stroke-linejoin="round" d="M4 14.899A7 7 0 1115.62 3.207M14.319 19a4 4 0 01-3.319 1M14 21V9M3 10l9-7 9 7" />' },
  CSSLLockers: { label: 'Secure Lockers', svg: '<path stroke-linecap="round" stroke-linejoin="round" d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z" />' },
  CSSLBenches: { label: 'Stable Seating', svg: '<path stroke-linecap="round" stroke-linejoin="round" d="M4 18v-4m0 0V8a2 2 0 012-2h12a2 2 0 012 2v6m0 0v4m-16-4h16" />' },
};
...
function CSSLPage() {
  ...
  const [deviations, setDeviations] = useState({
    CSSL: buildInitialDeviationState(CSSL_ITEMS),
  })
  ...
  async function handleSubmit(event) {
    ...
    const payload = {
      ...
      formPayload: {
        ...
        deviations: {
          CSSL: sectionToLabels(CSSL_ITEMS, deviations.CSSL, CSSL_CODES),
        },
      },
    }
  ...
  return (
    <section className="pb-32 flex flex-col items-center">
      <header className="w-full flex items-center justify-between px-4 pt-3 pb-4 bg-white/40 backdrop-blur-xl border-b border-white/50 shadow-sm rounded-none mb-4">
        <div className="flex items-center gap-3">
          ...
          <div className="flex flex-col">
            ...
            <span className="text-slate-900 font-bold text-base leading-tight">Change Rooms & Lockers</span>
          </div>
        </div>
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-5">
        ...
        {renderSection('CSSL', 'Change Rooms & Lockers Check', CSSL_ITEMS, CSSL_CODES)}
