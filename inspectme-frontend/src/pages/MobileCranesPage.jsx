...
const MC_CODES = [
  ...
  { value: 'MC1', label: 'MC1 - Obstructed exit doors' },
  { value: 'MC2', label: 'MC2 - Doors do not open outwards' },
  { value: 'MC3', label: 'MC3 - Missing/faulty panic hardware' },
  { value: 'MC4', label: 'MC4 - Combustible materials in escape routes' },
  { value: 'MC5', label: 'MC5 - Non-compliant signage' },
  { value: 'MC6', label: 'MC6 - Signage not illuminated/visible' },
  { value: 'MC7', label: 'MC7 - Backup lighting failed' },
  { value: 'MC8', label: 'MC8 - Assembly point issues' },
  { value: 'MC9', label: 'MC9 - Other' },
]

const MC_ITEMS = [
  { key: 'mc01', label: 'Are all designated emergency exit doors completely unobstructed on both sides?' },
  { key: 'mc02', label: 'Do all emergency exit doors open outwards in the direction of the escape route?' },
  { key: 'mc03', label: 'Are exit doors fitted with panic hardware that does not require a key to open from the inside?' },
  { key: 'mc04', label: 'Are all escape routes entirely free of combustible materials or temporary storage?' },
  { key: 'mc05', label: 'Are emergency exit signs strictly compliant with SANS 1186 (correct green/white symbolic safety signs)?' },
  { key: 'mc06', label: 'Is the emergency exit signage continuously illuminated and clearly visible from anywhere within the escape path?' },
  { key: 'mc07', label: 'Are backup emergency lighting systems functional and tested to operate during load shedding or power failures?' },
  { key: 'mc08', label: 'Are designated outdoor emergency assembly points clearly marked and safely accessible?' },
]

const ITEM_METADATA = {
  mc01: { label: 'Item 1', svg: '<path stroke-linecap="round" stroke-linejoin="round" d="M3 13h2l2 3h10l2-3h2M3 13v6h18v-6M5 13l2-5h10l2 5" />' },
  mc02: { label: 'Item 2', svg: '<path stroke-linecap="round" stroke-linejoin="round" d="M3 13h2l2 3h10l2-3h2M3 13v6h18v-6M5 13l2-5h10l2 5" />' },
  mc03: { label: 'Item 3', svg: '<path stroke-linecap="round" stroke-linejoin="round" d="M3 13h2l2 3h10l2-3h2M3 13v6h18v-6M5 13l2-5h10l2 5" />' },
  mc04: { label: 'Item 4', svg: '<path stroke-linecap="round" stroke-linejoin="round" d="M3 13h2l2 3h10l2-3h2M3 13v6h18v-6M5 13l2-5h10l2 5" />' },
  mc05: { label: 'Item 5', svg: '<path stroke-linecap="round" stroke-linejoin="round" d="M3 13h2l2 3h10l2-3h2M3 13v6h18v-6M5 13l2-5h10l2 5" />' }
};
...
function MobileCranesPage() {
  ...
  const [deviations, setDeviations] = useState({
    mc: buildInitialDeviationState(MC_ITEMS),
  })
  ...
  async function handleSubmit(event) {
    ...
    const sectionToLabels = (items, sectionValues, optionsMap) =>
      ...
      const payload = {
        ...
        employee: user.id,
        inspectionType: 'MOBILE_CRANES', // Matches naming convention
        formPayload: {
          details: {
            area: formData.area,
          ...
          deviations: {
            mc: sectionToLabels(MC_ITEMS, deviations.mc, MC_CODES),
          },
        },
      }
  ...
  return (
    <section className="pb-32 flex flex-col items-center">
      <header className="w-full flex items-center justify-between px-4 pt-3 pb-4 bg-white/40 backdrop-blur-xl border-b border-white/50 shadow-sm rounded-none mb-4">
        <div className="flex items-center gap-3">
          ...
          <div className="flex flex-col">
            ...
            <span className="text-slate-900 font-bold text-base leading-tight">Mobile Cranes</span>
          </div>
        </div>
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-5 px-4 w-full">
        ...
        {renderSection('mc', 'Mobile Cranes', MC_ITEMS, MC_CODES)}
...
export default MobileCranesPage
